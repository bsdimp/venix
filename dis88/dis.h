...
#ifdef VENIX
#define MAXSYM  ((sizeof(int)-1)*20000/ \
                 (sizeof(struct symtb)+sizeof(struct relocation_info)))
			/* Maximum entries in symbol table  */
extern struct symtb     /* Array to hold the symbol table   */
   symtab[MAXSYM];

extern struct relocation_info     /* Array to hold relocation table   */
   relo[MAXSYM];

/* #define n_name ns_un.ns_name */
#define n_value ns_value
#define N_IS_EXT(s)	((s).ns_type & N_EXT)
#define N_TYP(s)	((s).ns_type & N_TYPE)
#define R_IDX(r)	((r).r_symbolnum)
#define r_vaddr		r_address
#define BADMAG(x)	N_BADMAG(x)
#else
...
extern struct reloc     /* Array to hold relocation table   */
   relo[MAXSYM];
#define N_IS_EXT(s)	(((s).ns_sclass & N_CLASS) == C_EXT)
#define N_TYP(s)	((s).ns_sclass & N_SECT)
#define R_IDX(r)	((r).r_symndx)
#endif

/* XXX cleanup */
#define N_IS_COMM(s)	(N_TYP(s) <= N_UNDF || N_TYP(s) >= N_COMM)
