...
#ifdef VENIX
struct symtb symtab[MAXSYM];  /* Array of symbol table info */
struct relocation_info relo[MAXSYM]; /* Array of relocation info   */
extern char strtab[];
#else
...
{/* * * * * * * * * *  START OF getnam()  * * * * * * * * * */
#ifdef VENIX
   return strtab + symtab[k].ns_un.ns_strx;
//   return (symtab[k].ns_un.ns_name);
#else
   register int j;
